...
public class Temperaturas2025 {
    public static void main(String[] args) {
        Random rand = new Random(); // Crea un objeto Random para generar valores aleatorios
        int[] diasPorMes = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}; // Días de cada mes
        double[][] temperaturas = new double[12][]; // Matriz para almacenar temperaturas

        // Generar temperaturas aleatorias entre 30°C y 41°C
        for (int mes = 0; mes < 12; mes++) {
            temperaturas[mes] = new double[diasPorMes[mes]];
            for (int dia = 0; dia < diasPorMes[mes]; dia++) {
                temperaturas[mes][dia] = 30 + rand.nextDouble() * 11; // Genera valores entre 30 y 41
            }
        }
